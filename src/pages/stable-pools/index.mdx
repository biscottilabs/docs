---
title: Stable Pools
description: Curve-style StableSwap pools on Biscotti with near-zero slippage for like-priced assets.
---

# Stable Pools

Stable pools are Biscotti's second AMM: two-coin **StableSwap** pools built
for assets that should trade near parity, including stablecoin pairs like
USDC/EURC or wrapped/liquid-staked variants of the same asset.
...
- `D`: the invariant, representing the total value of the pool when balanced
- `A`: the **amplification coefficient**, which controls how strongly the pool
  behaves like a constant-sum curve near the peg
- `n = 2`: the number of coins in a Biscotti pool
...
- [Providing Liquidity](/stable-pools/providing-liquidity): deposits, withdrawals
