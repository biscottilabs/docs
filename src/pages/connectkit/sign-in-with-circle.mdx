...
| `appId` | None | Public Circle App ID. |
| `google.clientId` | None | Public Google OAuth Web client ID. |
...
| `endpoints.headers` | None | Static or generated headers for app authentication. |
| `endpoints.fetchOptions` | None | Extra `fetch` options such as credentials. |
| `adapter` | None | Replace the built-in HTTP adapter. |
